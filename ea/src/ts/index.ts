...
function createSatellite(scene: Scene) {
  // 구체로 인공위성 표현
  const satellite = MeshBuilder.CreateSphere("satellite", {diameter: 0.05}, scene);

  // 머티리얼 설정 (은색)
  const mat = new StandardMaterial("satMat", scene);
  mat.diffuseColor = new Color3(0.8, 0.8, 0.8);
  satellite.material = mat;

  // 지구 근처 위치 지정 (예: 지구 중심이 (0,0,0)일 때)
  satellite.position = new Vector3(0.3, 0.1, 0.3);
