...
#### 제어 버튼(WebSocket 메시지 전송) 매핑

    - 각 제어 버튼을 누르면 아래와 같이 지정된 번호의 메시지가 WebSocket을 통해 서버로 전송됩니다.
    - 이 메시지는 서버에서 시리얼 포트로 전달되어 실제 하드웨어 제어에 사용됩니다.
    ...
    - 각 버튼은 main.ts에서 아래와 같이 이벤트가 등록되어 있습니다:

     - 각 메시지는 서버에서 시리얼 포트로 전달되어 실제 장치 제어에 사용됩니다.
